...
/** Tiny, safe markdown renderer for guidebooks (headings, bold/italic, lists, tables). No HTML injection. */
...
export function Markdown({ source }: { source: string }) {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('### ')) blocks.push(<h4 key={i} className="mb-2 mt-5 text-lg font-extrabold">{inline(line.slice(4))}</h4>)
